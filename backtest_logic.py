...
GAME_CONFIG = {
    ...
    "5/35": {"type": "special",  "range": 35, "balls": 5, "special_range": 12, # 5 balls (1-35) + 1 ball (1-12)
...
def generate_simulation(game_type, limit=400):
    ...
    for _ in range(limit):
        if cfg["type"] == "standard":
            ...
            # Special: 1 ball from 1-12
            special_ball = random.randint(1, cfg["special_range"])
...
def advanced_pattern_filters(draw, game_type):
    ...
    return True
    # Range limit: cfg["range"]
    limit = 55 if game_type == "6/55" else (45 if game_type == "6/45" else 35)
    mid = limit // 2
    lows = len([n for n in draw if n <= mid])
    highs = len([n for n in draw if n > mid])
    # Extreme imbalance check (e.g. all low or all high)
    if lows == 0 or highs == 0: return False
...
def mode_a_ai_analysis(game_type, history, hot_nums, cold_nums, seed=None):
    ...
    """
    return generate_predictions(game_type, history, hot_nums, cold_nums, seed=seed)
...
def mode_f_hybrid(game_type, history, hot_nums, cold_nums, seed=None, quantum_dob=None, iching_dob=None, bio_score=0.0, bankers=None, style="Modern"):
    ...
    for n in votes:
        if n in fft_scores:
             # REDUCED: Multiplier dampened
             votes[n] *= (1.0 + (fft_scores[n] * 0.5))
    ...
    for n in votes:
        if n in echo_scores:
             # REDUCED: Was 5.0, now 1.5. Let core frequency speak.
             votes[n] *= (1.0 + (echo_scores[n] * 1.5 * w_history))
...
def main():
    ...
    with st.sidebar:
        st.header("Cấu hình")
        game_choice = st.selectbox("Loại vé:", ["6/55", "6/45", "5/35"])
        ...
        st.markdown("---")
        with st.expander("👤 Thông tin Cá nhân (Tùy chọn)"):
            st.caption("Nhập để AI tối ưu theo trường năng lượng của bạn.")
            default_dob = datetime(1989, 1, 1)
            u_iching_date = st.date_input("Ngày sinh:", value=default_dob)
            u_quantum_dob = u_iching_date # Sync for simplicity
            vip_iching_dob = datetime.combine(u_iching_date, datetime.min.time())
    ...
    tab1, tab2 = st.tabs(["🥇 AI PHÂN TÍCH (SINGLE)", "🕸️ MA TRẬN BAO VÂY (SYSTEM)"])
    
        # --- MODE 1: HYBRID SINGLE PICK ---
    with tab1:
        st.subheader("Dự đoán 1 bộ số tối ưu nhất")
        st.caption("Sử dụng thuật toán **Hybrid (AI + Lượng tử + Kinh dịch)** để tìm ra bộ số có sác xuất cao nhất.")
        ...
        with col_chaos1:
             st.info(f"⚡ Mức độ hỗn loạn (Entropy): {st.session_state.chaos_factor}")
        with col_chaos2:
             if st.button("🔄 Đảo chiều (Re-roll)"):
        ...
        if st.button("KÍCH HOẠT PHÂN TÍCH CHUYÊN SÂU", type="primary", key="btn_hybrid"):
            progress_bar = st.progress(0)
            status_text = st.empty()
            ...
            result_numbers, details = mode_f_hybrid(
            ...
            st.success("Đây là bộ số có sự đồng thuận cao nhất từ các thuật toán.")
